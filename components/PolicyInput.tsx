...
export default function PolicyInput({ policy, onPolicyChange, onRun, isRunning }: Props) {
  return (
    <div className="rounded-lg border border-gray-200 bg-white p-5 mb-6 shadow-sm">
      <div className="mb-3 text-sm font-semibold text-gray-800">公約・政策を入力</div>
      <textarea
        value={policy}
        onChange={(e) => onPolicyChange(e.target.value)}
        placeholder="例：子育て世帯への月額1万円の給付金を新設し、18歳まで所得制限なしで支給します"
        className="w-full min-h-[80px] rounded-md border border-gray-300 bg-white p-3 text-sm leading-relaxed text-gray-900 outline-none resize-y focus:ring-2 focus:ring-[#1B2A4A] focus:border-[#1B2A4A] placeholder:text-gray-400"
      />
